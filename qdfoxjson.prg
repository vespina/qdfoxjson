...
* -------------------------------------------------------------------------
* VERSION HISTORY
* VES	May 29, 2014	1.9		- Propiedad stringDelimitator pasa a valor ["] por omision
*							    - Cambio en metodo _Split para tomar en cuenta "scaped chars"
*
 ...
 * Class constructor
 PROCEDURE Init
  ...
  THIS.oFJE = CREATEOBJECT("fastJSON")
 ENDPROC
	 ...
	 *
     ...
     DO CASE
	    ...
	    OTHERWISE 
	         ...
	         *#ENDIF
	         RETURN THIS.oFJE.parseJSON(pcJSON)
	 ENDCASE
 ...
 *******************************************
  ...
  *lIsArray = (TYPE("puValue",1) = "A")
  ...
  DO CASE
     ...
     CASE cType $ "NIYF"   && Numeric value
          ...
          ELSE
           cJSONValue = CHRTRAN(RTRIMX(RTRIMX(TRANSFORM(puValue), "0"), SET("POINT")),SET("POINT"), ".")  && JuanPa / Rafel Cano, Abril 13 2012
          ENDIF
 ...
 HIDDEN PROCEDURE _Split(pcJSON)
	 *
	 LOCAL nBlockCount,cObj,lOpenQuote,cChar,cLastChar
	 nBlockCount = 0  
	 cObj = pcJSON
	 cLastChar = CHR(0)
	 lOpenQuote = .F.
	 FOR j = 1 TO LEN(cObj)
	   ...
	   DO CASE
	      ...
	      CASE cChar $ THIS.stringDelimitator AND cLastChar<>"\"
	           IF lOpenQuote
	            nBlockCount = nBLockCount - 1
	           ELSE
	            nBlockCount = nBlockCount + 1 
...
*
DEFINE CLASS fastJSON AS Custom

	PROCEDURE ParseJSON(pcJSON, pnPos)
	 *
	 IF PCOUNT() < 2
	 ...
	 LOCAL i, nLen, cChar, cLastChar, nMode, cStringSep, lIsString, cProp, nNestLevel, oValue, lExitLoop, oTarget, uBuff, lIsArray, oArray
	 nLen = LEN(pcJSON)
	 cLastChar = CHR(0)
	 oTarget = NULL
	 cStringSep = ["]
	 ...
	 lIsArray = (cChar = "[")
	 oTarget = IIF(lIsArray, CREATEOBJECT("Collection"), CREATEOBJECT("Empty"))
	  ...
	  *
	  ...
	  DO CASE
	     CASE cChar = "{" AND !lIsString  && Start new object
	          uBuff = THIS.ParseJSON(pcJSON, @i)
	          nMode = 3
	          
	     CASE cChar $ "}]" AND !lIsString  && Close last object or array
	          lExitLoop = .T.
	          nMode = IIF(INLIST(cLastChar,cStringSep,"{","[") OR (EMPTY(cProp) AND !lIsArray), nMode, 3)
	...
	PROCEDURE toJSON(puObjectOrAlias)
	 *
	 #DEFINE VFP_NOENCODABLE_PROPS	"-controls-controlcount-objects-parent-class-baseclass-classlibrary-parentclass-helpcontextid-whatsthishelpid-top-left-width-height-picture-_customproplist-"
	 
	 LOCAL cJSON,cType,oRow,nWkArea,nRow,nCount,i,j,cProp,uValue,cValue,cValueType
	 LOCAL ARRAY aProps[1]
	 cJSON = ""
	 nWkArea = SELECT()
	 cType = VARTYPE(puObjectOrAlias)
	 lIsArray = (cType = "O" AND PEMSTATUS(puObjectOrAlias,"class",5) AND LOWER(puObjectOrAlias.Class) == "collection")
	 DO CASE
	    ...
	    CASE cType = "O" AND lIsArray     && Array
	         ...
	         FOR i = 1 TO puObjectOrAlias.Count
	          cJSON = cJSON + IIF(i > 1,",","") + THIS.ToJSON(puObjectOrAlias.Item[i])
	         ENDFOR
	         cJSON = cJSON + "]"
	    
	    
	    
	    CASE cType = "C" AND USED(puObjectOrAlias)   && Cursor
	         ...
	         SCAN
	          nRow = nRow + 1
	          SCATTER NAME oRow MEMO
	          cJSON = cJSON + IIF(nRow > 1,",","") + THIS.toJSON(oRow)
	         ENDSCAN
	         cJSON = cJSON + "]}"
	    ...
	    OTHERWISE      && Scalar value
		      ...
		      DO CASE
		         ...
		         CASE cValueType = "D"
		              cValue = ["] + STR(YEAR(uValue),4) + "-" + PADL(MONTH(uValue),2,"0") + "-" + PADL(DAY(uValue),2,"0") + ["]
